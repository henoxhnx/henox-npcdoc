...
RegisterCommand("doktor", function(source, args, raw)
	if (QBCore.Functions.GetPlayerData().metadata["isdead"]) or (QBCore.Functions.GetPlayerData().metadata["inlaststand"]) and spam then
		QBCore.Functions.TriggerCallback('henox-npcdoc:docOnline', function(EMSOnline, hasEnoughMoney)
			if EMSOnline <= Config.Doctor and hasEnoughMoney and spam then
				SpawnVehicle(GetEntityCoords(PlayerPedId()))
				TriggerServerEvent('henox-npcdoc:charge')
				Notify("Doktor yolda.")
			else
				if EMSOnline > Config.Doctor then
					Notify("Çevrimiçi çok fazla doktor var", "error")
				elseif not hasEnoughMoney then
					Notify("Yeterli paran yok", "error")
				else
					Notify("Lütfen bekleyin. Doktor yolda.", "primary")
				end	
			end
		end)
	else
		Notify("Bu yalnızca öldüğünde kullanılabilir", "error")
	end
end)
...
function SpawnVehicle(x, y, z)  
	...
	if not DoesEntityExist(vehhash) then
		...
		SetVehicleNumberPlateText(mechVeh, "HENoX")
...
function DoctorNPC()
	...
	QBCore.Functions.Progressbar("revive_doc", "Doktor sana tıbbi yardımda bulunuyor", Config.ReviveTime, false, false, {
		...
		Notify("Tedaviniz bitti, tedavi ücreti : "..Config.Price, "success")
